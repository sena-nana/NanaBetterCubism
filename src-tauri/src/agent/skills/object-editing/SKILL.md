...
4. After round approval, call `execute_editor_edit` once per confirmed same-method batch, then poll `get_editor_edit_result`. Report success only when the entire batch is committed and every item is verified. Do not split a batch into per-item transactions or ask again per edit within the same round; if the plan changes mid-round, stop and re-confirm via ask_user before proceeding.

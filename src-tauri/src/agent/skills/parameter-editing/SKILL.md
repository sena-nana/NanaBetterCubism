...
4. Gather every preview needed this turn, summarize all previews together, and obtain a single round approval via ask_user describing all operations at once, unless the current request already authorizes that exact set of changes.
5. After round approval, execute every confirmed official preview with `execute_editor_edit`, poll `get_editor_edit_result` until it is no longer running, and report success only for a verified committed outcome. Do not ask again per edit within the same round; if the plan changes mid-round, stop and re-confirm via ask_user before proceeding.

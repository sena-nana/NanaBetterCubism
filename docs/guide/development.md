...
- Vue 只调用类型化 Tauri 命令（对话、记忆管理、LLM 配置、Editor 连接领域命令）。
...
- Agent ReAct 循环、API Key（keyring）、SQLite 会话/记忆与 Agent 记忆作用域校验均在 Rust。
- 新对话由 Rust 通过当前建模文档路径自动归类；未保存或无法识别的文档进入收集箱，已有对话不会随 Editor 切换而迁移。
- 提供 `ask_user` / `update_plan`；不做权限批准门，不提供文件直接编辑工具。
- Agent 初始只获得连接、提问、计划与 `read_skill` 基础工具；记忆不自动注入上下文，只有按需读取 `project-memory` 后才开放 `list_memories` / `read_memory` / `upsert_memory` / `archive_memory`。记忆正文为固定多层 Markdown（project：Overview/Stage/Structure/Decisions；global：Summary/Technique/Caveats），列表仅返回索引层，深层按需读取。运行时 SKILL 在等待用户回答后继续保留，新消息开始时重置。

...
- Agent 初始只获得连接、提问、计划与 `read_skill` 基础工具；记忆不自动注入上下文。`memory-recall` 仅开放本地确定性 `recall_memory`，由 Rust 按 `index` / `focused` / `full` 深度匹配固定多层 Markdown；`project-memory` 仅开放保存与归档。运行时 SKILL 在等待用户回答后继续保留，新消息开始时重置。
